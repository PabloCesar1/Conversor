...
public class MainActivityPE extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!tmpMonedaActual.equals("")){
            int indice = adaptador.getPosition(tmpMonedaActual);//obtenemos la posicion del adaptador
            monedaActualSP.setSelection(indice);//declaramos la posicion en el spinner
        }
        if (!tmpMonedaCambio.equals("")){
            int indice = adaptador.getPosition(tmpMonedaCambio);//obtenemos la posicion del adaptador
            monedaCambioSP.setSelection(indice);//declaramos la posicion en el spinner
        }
    ...
    public void clickConvertir(View v){
        ...
        if (resultado > 0){
            ...
            Toast.makeText(MainActivityPE.this, "Las opciones elegidas no tienen un factor de conversión", Toast.LENGTH_SHORT).show();
        }
    }
